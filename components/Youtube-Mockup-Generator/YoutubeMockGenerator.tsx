...
import { initialYoutubedata, Youtubedata } from "@/lib/types";
import { YoutubeCard } from "./youtube-card";
import { YoutubeDataForm } from "./youtube-data-form";
import { Download, RefreshCw, ArrowLeft } from "lucide-react";
import { toPng } from "html-to-image";
import Link from "next/link";

export default function YoutubeMockGenerator() {
  const [data, setdata] = useState<Youtubedata>(initialYoutubedata);
  const [isExporting, setIsExporting] = useState(false);

  const handleExport = async () => {
    const node = document.getElementById("youtube-card");
    if (!node) return;
    ...
    try {
      const dataUrl = await toPng(node, {
        quality: 1.0,
        pixelRatio: 3,
        backgroundColor: data.theme === "light" ? "#ffffff" : "#0f0f0f",
      });
      ...
      console.error("Failed to export YouTube mockup image", err);
    } finally {
      ...
      <header className="border-b bg-white dark:bg-[#0a0a0a] border-gray-200 dark:border-zinc-800 p-4 sticky top-0 z-10">
        <div className="max-w-6xl mx-auto flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <h1 className="text-xl font-light bg-gradient-to-r from-red-650 to-red-500 bg-clip-text text-transparent">
              YouTube Mockup Generator
            </h1>
          </div>
          <button
            onClick={handleExport}
            disabled={isExporting}
            className="flex items-center gap-2 px-4 font-light py-2 bg-[#ff0000] hover:bg-[#e60000] text-white rounded-full transition-transform active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed text-sm shadow-md"
          >
            {isExporting ? (
      ...
      <main className="max-w-6xl mx-auto p-4 md:p-8 flex flex-col lg:flex-row gap-8">
        ...
        <section className="flex-1 flex flex-col items-center lg:items-start lg:sticky lg:top-24 h-fit">
          ...
          <div className="mt-6 text-sm text-gray-400 text-center lg:text-left max-w-md">
            Adjust the settings on the right to customize your YouTube video watch page. All changes appear in real-time in high-fidelity.
          </div>
        </section>
        ...
        <section className="flex-1 bg-white dark:bg-[#16181c] rounded-2xl shadow-sm border border-gray-200 dark:border-zinc-800 h-fit overflow-hidden">
          ...
          <YoutubeDataForm data={data} onChange={setdata} />
        </section>
      </main>
    </div>
  );
